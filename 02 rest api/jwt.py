...
# Получение пользователя из clien_db
def get_user_from_db(client_db, username: str):
    if username in client_db:
        user_dict = client_db[username]
        return User(**user_dict)
        
# Аутентификация
def authenticate_user(client_db, username: str, password: str):
    user= get_user_from_db(client_db, username)
    if not user:
        return False
...
async def get_current_client(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        raise credentials_exception
    user = get_user_from_db(client_db, username=username)
    if user is None:
        raise credentials_exception
    return user
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    user = authenticate_user(client_db, form_data.username, form_data.password)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
